...
Not handled: scanned/image-only PDFs (no text layer -> needs OCR) and multi-
column layouts may interleave (pdfplumber's own ``extract_text`` has the same
limitation).
    ...
    *,
    ...
    with _open(source, password) as pdf:
        for page in pdf.pages:
            ...
            for line_words in _group_lines(words, y_tolerance):
